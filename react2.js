let cliSelect = require('cli-select')
...
function noteOn(message, delta) {
  ...
  let brightness = Math.max((vel / 127) ** 3, 0.2)
  brightness = 100

  let color = Color.rgb(255, 0, 255).rotate(timer * 40).value(brightness)
  let reverbColor = color.rotate(180).value(reverbMix * reverb.mix * 100)

  console.log(reverbColor.toString())
  
  reverb.spawn(note, reverbColor)
  array[note] = color
}
...
function write(keyboard, colors) {
  let data = []
...
function loop() {
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    a.push(add(array[i], reverb.array[i]))
  }
  write(a)
